...
# Example usage
file1_path = 'output2.txt'  # Path to the first file
file2_path = 'file3.txt'  # Path to the second file
...
if num_different_lines is not None:
    ...
    print("MED: {:.2f}".format((modulus_sum/total_lines)))
    print("NMED: {:.6f}".format(((modulus_sum/total_lines)/((2**16 - 1)**2))))
    print("MRED: {:.6f}".format(((modulus_sum2/(2*total_lines)))))
    result = 32 - math.log(1 + ((modulus_sum_square / total_lines) ** 0.5), 2)
